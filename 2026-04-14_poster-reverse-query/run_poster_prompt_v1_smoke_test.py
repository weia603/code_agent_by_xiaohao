#!/usr/bin/env python3
"""Run poster_prompt_v1 on a small batch of poster/infographic schema summaries.
...
PROMPT_FILE = Path(__file__).resolve().parent / 'poster_prompt_v1.md'
...
async def main_async():
    ...
    result_path = output_dir / f'result_poster_prompt_v1_smoke_{ts}.jsonl'
    error_path = output_dir / f'errors_poster_prompt_v1_smoke_{ts}.jsonl'
    stats_path = output_dir / f'stats_poster_prompt_v1_smoke_{ts}.json'
    ...
    # Reuse build script call path by patching prompt globals at runtime.
    ...
    try:
        ...
        if system_split in prompt_text:
            ...
            build_mod.SYSTEM_PROMPT = system_part.replace('# poster_prompt_v1', '').replace('## System prompt', '').strip()
